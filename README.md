...
## V72 Guided Studio Redesign

Major UX surface redesign.

Changed:

- Added Guided Mascot Studio overview.
- Reframed the app around Start → Draw → Polish.
- Renamed confusing sections for clarity.
- Hid optional extra labs by default behind an Extra Labs toggle.
- Kept all existing features and logic intact.
- Reduced visual overwhelm with clearer hierarchy and calmer panel language.
- Preserved glass, glow, recipe cards, practice tools, universal blueprint restore, and Shade Lab amplifiers.

Service worker cache:

```js
const CACHE_NAME = "mascot-spark-v72";
```
